...
authors: Rebecca Hinrichs & teammate
...
```{r}
world_population <- read.csv("PopulationByAgeSex.csv")    # import data frame provided at Kaggle (see citations)
```

First, we'll make one small adjustment. The data was imported with a (.) in the thousands place of all populations in order to represent population by millions. For our purposes, we'll move the decimal to the standard ones place.
```{r}
# Create new data frame from original by multiplying all 'Pop*' (population) columns by 1000
...
```{r}
# Arrange data by Location, then count number of distinct Locations
...
```{r}
my_countries_data %>% group_by(Location) %>% count() %>% head(1)
```
...
```{r}
fewer_than <- my_countries_data %>% group_by(Location) %>% 
  filter(n() < 151) %>% nrow()
...
```{r}
set.seed(10)
random_countries <- my_countries_data[sample(nrow(my_countries_data), 
                                             size = 10), ]
random_countries %>% 
  select(Id, LocID, Location, Time, PopMale, PopFemale, PopTotal) %>% 
...
When we look at the entire growth of the world population graphically, we observe a classic exponential curve. Notice that the slope of the curve is steepest around the millenium and begins to flatten around 2050. The points represent every 25 years.
```{r}
# World population from 1950 to 2100 plot
my_countries_data %>% 
  filter(Time %in% c(1950, 1975, 2000, 2025,2050,2075,2100) ) %>% 
  group_by(Time) %>% summarise( WorldPopulation = sum(PopTotal)) %>% 
  ggplot(aes(x = Time, y = WorldPopulation))+ 
  geom_point(color = 'red', size = 2) + 
  geom_line(color = 'dark green', size = 1) + 
  labs(title = 'Global Population from 1950 to 2100', x = 'Year', y = 'Population')
```
...
```{r}
# World population from 1950 to 2100
my_countries_data %>% 
  filter(Time %in% c(1950, 1975, 2000, 2025, 2050, 2075, 2100) ) %>% 
  group_by(Time) %>% summarise(WorldPopulation = sum(PopTotal)) %>% 
  kbl(caption = "Population per 25 Years") %>%
  kable_classic_2(full_width = F, html_font = "Cambria")
```
...
```{r}
table_1950 <- my_countries_data %>% filter(Time == '1950') %>% 
  arrange(desc(PopTotal)) %>% select(LocID, Location, PopTotal) %>% head(10)

table_2021 <- my_countries_data %>% filter(Time == '2021') %>% 
  arrange(desc(PopTotal)) %>% select(LocID, Location, PopTotal) %>% head(10)

table_2050 <- my_countries_data %>% filter(Time == '2050') %>% 
  arrange(desc(PopTotal)) %>% select(LocID, Location, PopTotal) %>% head(10)

table_2100 <- my_countries_data %>% filter(Time == '2100') %>% 
  arrange(desc(PopTotal)) %>% select(LocID, Location, PopTotal) %>% head(10)
...
```{r}
top_1950 <- my_countries_data %>% filter(Time == '1950') %>% arrange(desc(PopTotal)) %>% 
  select(Location, PopTotal) %>% head(10) %>% 
  mutate(Location = fct_reorder(Location, PopTotal)) %>% 
  ggplot(aes(x = PopTotal , y = Location)) + 
...
top_2021 <- my_countries_data %>% filter(Time == '2021') %>% arrange(desc(PopTotal)) %>% 
  select(Location, PopTotal) %>% head(10) %>% 
  mutate(Location = fct_reorder(Location, PopTotal)) %>% 
  ggplot(aes(x = PopTotal , y = Location)) + 
...
top_2050 <- my_countries_data %>% filter(Time == '2050') %>% arrange(desc(PopTotal)) %>% 
  select(Location, PopTotal) %>% head(10) %>% 
  mutate(Location = fct_reorder(Location, PopTotal)) %>% 
  ggplot(aes(x = PopTotal , y = Location)) + 
  geom_col(fill = 'blue')+
...
top_2100 <- my_countries_data %>% filter(Time == '2100') %>% arrange(desc(PopTotal)) %>% 
  select(Location, PopTotal) %>% head(10) %>% 
  mutate(Location = fct_reorder(Location, PopTotal)) %>% 
  ggplot(aes(x = PopTotal , y = Location)) + 
...
Our bar graphs make it pretty clear how strong of an influence the top contributors to world population are currently, but we can see a significant change coming by 2100, where those top contributors decrease not just in growth but in population size, while the smaller contributors will have experienced much more growth by then. Most notably, in 2021, we see that China has held the top position for the largest contributor to world population, and that India is projected to hold that title by 2050 and in 2100. 
In which years will India have the largest population in the world?
```{r}
my_countries_data %>% select(Time, Location, PopTotal) %>% group_by(Time) %>%  
...
```
India will hold the top contributor ranking from 2027 through 2100.
...
```{r}
my_countries_data %>% filter(Time == '2021') %>% 
  arrange(desc(PopTotal)) %>% 
  select(Location, PopTotal) %>%
  head(5) %>% 
  kbl(caption = "Top 5 Population Contributors 
      by Country in 2021") %>%
  kable_classic_2(full_width = F, html_font = "Cambria") %>% 
...
my_countries_data %>% filter(Location %in% top_countries) %>% 
  ggplot(aes(x = Time, y = PopTotal, color = Location)) + geom_line() + 
...
Let's investigate whether these shifts happen by gender or by age group. First we'll take a look at the Total Male Population among these top 5 countries. 
```{r}
#Total male population of 5 selected countries from 1950 to 2100
my_countries_data %>% filter(Location %in% top_countries) %>% 
  ggplot(aes(x = Time, y = PopMale, color = Location)) + geom_line() +
  scale_x_continuous(breaks = c(1950,2000,2021,2050,2100))+
  geom_vline(xintercept = 2021, color = 'blue', linetype = 'longdash') +
  labs(title = 'Total Male Population from 1950 to 2100',
       subtitle ='Countries: China, India, United States of America, Indonesia, Pakistan', 
       x = 'Year', y = 'Population',colour = 'Country')
...
We see that the shifts in total male population appear to mirror exactly that of the total population, so we can assume the same will be true for the total female population.
...
```{r}
top_5 <- my_countries_data %>% filter(Location %in% top_countries)   # we'll use this vector later as well

top_5 %>% filter(Time >= 2021) %>% 
  mutate(under_35 = PopTotal_0_4 + PopTotal_5_9 + PopTotal_10_14 + 
           PopTotal_15_19 + PopTotal_20_24 + PopTotal_25_29 + PopTotal_30_34) %>%
  select(Time, Location, PopTotal, under_35) %>% 
  mutate(prop_under_35 = under_35/ PopTotal * 100) %>% 
  ggplot(aes(x = Time, y = prop_under_35, color = Location)) +
  geom_line() +
...
top_5 %>% filter(Time >= 2021) %>% 
  ...
  select(Time, Location, PopTotal, over_65) %>% 
  mutate(prop_over_65 = over_65/ PopTotal * 100) %>% 
  ggplot(aes(x = Time, y = prop_over_65, color = Location))+
  geom_line()+
...
data <- my_countries_data %>% filter(Time %in% c(2021, 2100)) %>% select(Time, Location, PopTotal) 

# Calculate the total change in population of each country
pop_diff <- data %>% group_by(Location) %>%
  mutate(difference = (PopTotal - lag(PopTotal))) %>% 
  select(Location, difference) %>% 
  na.omit()
...
projected_increase <- data %>% filter(Time == 2021) %>% 
  full_join(pop_increase, by = 'Location') %>% 
  mutate(percent_diff = round((difference/PopTotal*100),digits = 2)) %>% 
  arrange(desc(percent_diff))
projected_increase %>% select(Location,'Percentage Increase' = percent_diff) %>% 
  head(5) %>% 
  kbl(caption = "Top 5 Countries with Highest Projected Population Growth") %>%
  kable_classic_2(full_width = F, html_font = "Cambria") %>% 
  kable_styling(bootstrap_options = "striped", "hover")
```
...
What will the population of Niger look be in 2021 and in 2100?
```{r}
data %>% filter(Location == 'Niger') %>% 
  kbl(caption = "Current & Projected Population of Niger") %>%
  kable_classic_2(full_width = F, html_font = "Cambria") %>% 
  kable_styling(bootstrap_options = "striped", "hover")
```

What might that slope look like graphically?
```{r}
my_countries_data %>% 
  filter(Location == 'Niger' & Time %in% c(2021, 2040,2060, 2080,2100)) %>%
  ggplot(aes(x = Time, y =PopTotal)) + geom_point()+ geom_line()+
  labs(title = 'Population of Niger from 2021 to 2100', x = 'Year', y = 'Population')
```

How will Niger measure up against the next top 20 contributing countries with the strongest increase in population growth? Based on our chart below, those countries which follow Niger most closely are also identifiable as developing countries on the UN's list.
```{r}
projected_increase %>% arrange(desc(percent_diff)) %>%
  head(20) %>% 
  mutate(Location = fct_reorder(Location, percent_diff)) %>%
  ggplot(aes(x= percent_diff, y = Location, fill = PopTotal))+
  geom_col()+
  labs(title = 'Top 20 Countries with Projected Increase 
       Between 2021 and 2100', x = 'Percentage Increase', y = 'Country',
...
```{r}
my_countries_data %>% select(Time, PopTotal_0_4) %>% 
  group_by(Time) %>% 
  summarise(Total_0_4 = sum(PopTotal_0_4)) %>%
  ggplot(aes(x = Time, y = Total_0_4)) + geom_line() +
  scale_x_continuous(breaks = c(1950,2000,2021,2050,2100))+
...
The data makes it clear that live birth contributions to the world population is expected to decrease. Therefore, though we'll see a large push from growth contributors like Niger, the sharp decline in new births from our current contributors like China will, overall, decrease the global population significantly by 2050, according to our graph. New births appear projected to reach millenium levels by 2100. If the downward slope continues its course, could we expect 1950 levels to be reached within the next 80 years?
...
#Calculate to get top 10 countries with most decreasing in population
projected_decrease <- data %>% filter(Time == 2021) %>% 
  inner_join(dec_countries, by = 'Location') %>% 
  mutate(percent_decrease = round((-difference/PopTotal*100), digits = 2)) %>%
  arrange(desc(percent_decrease)) %>% select(Location, PopTotal, percent_decrease)
projected_decrease %>% select(Location, percent_decrease) %>%  head(5) %>% 
  kbl(caption = "Top 5 Countries with Most Decrease in Population") %>%
  kable_classic_2(full_width = F, html_font = "Cambria") %>% 
  kable_styling(bootstrap_options = "striped", "hover")
```
...
```{r}
data %>% filter(Location == 'Albania')
my_countries_data %>% filter(Location == 'Albania' & Time %in% c(2021, 2040,2060, 2080,2100)) %>%
  ggplot(aes(x = Time, y =PopTotal)) + geom_point()+ geom_line()+
  labs(title = 'Population of Albania from 2021 to 2100', x = 'Year', y = 'Population') %>% 
  kbl(caption = "Current & Projected Population of Albania") %>%
  kable_classic_2(full_width = F, html_font = "Cambria") %>% 
  kable_styling(bootstrap_options = "striped", "hover")
```

How will Albania compare with the next top 20 countries with the biggest projected decrease in population percentage?
```{r}
projected_decrease %>% arrange(desc(percent_decrease)) %>% head(20) %>% 
  mutate(Location = fct_reorder(Location, percent_decrease)) %>%
  ggplot(aes(x= percent_decrease, y = Location, fill = PopTotal))+
  geom_col()+
...
male_female <- my_countries_data %>% group_by(Time) %>% 
  summarise(total_female = sum(PopFemale), total_male = sum(PopMale)) 
male_female %>% 
  filter(row_number() %% 15 == 1) %>% 
  kbl(caption = 'Female v Male Population per 15yrs') %>%
  kable_classic_2(full_width = F, html_font = "Cambria")

# Graph plotting quantity of male versus female population by year
male_female %>% 
  ggplot(aes(x = total_female, y = total_male, color = Time)) + 
  geom_point() +
  labs(title = 'Total Female versus Male population from 1950 to 2100', 
       x = 'Total Female',
       y = 'Total Male')
```
...
Finally, in basic interest of our current environment, we'll take a look into what the United States' contribution will look like over the entire time period of 1950 to 2100. Our historic through projected growth appears nearly linear with a slowing growth pattern where our slope begins to fall again slightly. As we saw earlier in this report, this declining growth appears to be consistent with a larger aging population and a smaller new birth population. Therefore, it may not be unreasonable to anticipate our slope to continue to fall in years beyond 2100, though ongoing increase should still be expected.
```{r}
my_countries_data %>% filter(Location == 'United States of America') %>% 
  ggplot(aes(x = Time, y = PopTotal)) + geom_line() +
  scale_x_continuous(breaks = c(1950,2000,2021,2050,2100))+
  geom_vline(xintercept = 2021, color = 'hot pink', linetype = 'longdash') +
  labs(title = 'United States of America Population from 1950 to 2100', x = 'Year', y = 'Population')
```
